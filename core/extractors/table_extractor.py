"""
Table Extractor — Extract, validate, score, and rank HTML tables.
"""
...
def extract_tables(soup: BeautifulSoup, html: str) -> Dict[str, Any]:
    ...
    try:
        raw_tables = pd.read_html(io.StringIO(html), header=0, thousands=',', decimal='.', na_values=['-', 'N/A', ''])
    except ValueError:
    ...
    for idx, df in enumerate(raw_tables):
        try:
            ...
            table_data = {
                "index": idx + 1, "score": score, "rows": len(df), "columns": len(df.columns),
    ...
    return {"count": len(ranked), "total_found": len(raw_tables), "tables": ranked}


def _is_valid_table(df):
    rows, cols = df.shape
    ...
    if pd.isna(missing_ratio) or missing_ratio > 0.8:
        return False
    bad_headers = sum(1 for col in df.columns if "unnamed" in str(col).lower())
...
def _score_table(df):
    ...
    named_cols = sum(1 for col in df.columns if "unnamed" not in str(col).lower())
    score += named_cols * 5
    return max(score, 0)
