...
def extract_navigation(soup: BeautifulSoup, base_url: str) -> Dict[str, Any]:
    ...
    logger.info(
        ...
        f"{len(data['stylesheets'])} CSS, "
        f"{len(data['scripts'])} JS"
    )

    return data


def _get_nav_menus(soup: BeautifulSoup, base_url: str) -> List[Dict[str, Any]]:
    """Extract navigation menus from <nav> tags."""
    ...
    for nav in soup.find_all("nav"):
        ...
        nav_label = (
            nav.get("aria-label", "")
            or nav.get("id", "")
            or nav.get("class", [""])[0] if nav.get("class") else ""
        )
...
def _get_breadcrumbs(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
...
def _get_stylesheets(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
    """Extract CSS stylesheet references."""
...
def _get_scripts(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
    """Extract JavaScript file references (external only)."""
