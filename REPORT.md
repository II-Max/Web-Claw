...
*   **P0 — Critical Security**: In `core/extractors/table_extractor.py`, `pd.read_html` lacked the `flavor` parameter, defaulting to `lxml`, exposing the application to XML External Entity (XXE) vulnerabilities.
...
*   **`core/extractors/table_extractor.py`**:
    *   **Reason**: Prevent XML External Entity (XXE) injection vulnerabilities.
    *   **Modification**: Added `flavor='bs4'` to `pd.read_html()`.
    *   **Impact & Risks**: High security benefit. Eliminates XXE risks by forcing pandas to use BeautifulSoup instead of lxml for parsing HTML tables.
...
## Security Report
*   **Detected vulnerabilities**: Man-in-the-Middle (MITM) vulnerability due to disabled TLS verification (`verify=False`). XML External Entity (XXE) vulnerability in pandas HTML parsing.
*   **Applied fixes**: Enforced standard certificate validation in `core/scraper.py`. Added `flavor='bs4'` to `pd.read_html` in `core/extractors/table_extractor.py` to prevent XXE.
...
## Testing Report
*   **New tests**: Added `tests/test_cleaner.py` to cover the `extract_main_content` and `extract_clean_text` functions. Added `tests/test_table_extractor.py` to verify the table extraction process.
*   **Updated tests**: N/A
*   **Coverage impact**: Introduced the foundational testing framework. The core cleaning logic is now verified against XSS payload stripping and accurate target DOM extraction. Table extraction is verified to prevent parsing errors and regressions.
