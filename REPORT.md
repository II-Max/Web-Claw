...
*   **P0 — Critical Security**: In `core/extractors/table_extractor.py`, `pd.read_html` did not specify the `flavor` argument, defaulting to `lxml`, which creates an XML External Entity (XXE) vulnerability when parsing untrusted HTML tables.
...
*   **`core/extractors/table_extractor.py`**:
    *   **Reason**: Prevent XXE (XML External Entity) attacks when processing untrusted HTML tables.
    *   **Modification**: Added `flavor='bs4'` argument to `pd.read_html` to enforce BeautifulSoup parsing.
    *   **Impact & Risks**: Critical security benefit. No compatibility risk since `beautifulsoup4` and its parsers are already standard in the project.
...
*   **Detected vulnerabilities**:
    ...
    *   XML External Entity (XXE) vulnerability in `pandas.read_html` due to default `lxml` parser.
*   **Applied fixes**:
    *   Enforced standard certificate validation in `core/scraper.py`.
    *   Enforced `flavor='bs4'` in `core/extractors/table_extractor.py` to use a safe HTML parser.
...
*   **New tests**:
    ...
    *   Added `tests/test_table_extractor.py` to ensure tables are correctly extracted using the safe parser.
*   **Updated tests**: N/A
*   **Coverage impact**: Introduced the foundational testing framework. The core cleaning logic is now verified against XSS payload stripping and accurate target DOM extraction. Table extraction is now covered with tests for accuracy and bounds-checking.

## Changelog
*   `core/scraper.py`: Removed `verify=False` to fix TLS validation vulnerability.
*   `core/extractors/table_extractor.py`: Added `flavor='bs4'` to fix XXE vulnerability.
