...
*   **P0 — Critical Security**: In `core/extractors/table_extractor.py`, `pandas.read_html` was using the default parser, which can default to `lxml`. This makes the application vulnerable to XML External Entity (XXE) attacks when parsing untrusted HTML tables.
...
*   **`core/extractors/table_extractor.py`**:
    *   **Reason**: Prevent XXE vulnerabilities during HTML table parsing.
    *   **Modification**: Added `flavor='bs4'` to `pd.read_html` to enforce BeautifulSoup parsing.
    *   **Impact & Risks**: High security benefit. No compatibility risks as BeautifulSoup is already heavily used and strictly enforced.
...
## Security Report
*   **Detected vulnerabilities**: Man-in-the-Middle (MITM) vulnerability due to disabled TLS verification (`verify=False`). XML External Entity (XXE) vulnerability via default `lxml` parsing in pandas.
*   **Applied fixes**: Enforced standard certificate validation in `core/scraper.py`. Enforced BeautifulSoup parsing (`flavor='bs4'`) in `pandas.read_html` inside `core/extractors/table_extractor.py`.
...
## Testing Report
*   **New tests**: Added `tests/test_cleaner.py` to cover the `extract_main_content` and `extract_clean_text` functions. Added `tests/test_table_extractor.py` to cover table extraction logic and enforce parser stability.
*   **Updated tests**: N/A
*   **Coverage impact**: Introduced the foundational testing framework. The core cleaning logic is now verified against XSS payload stripping and accurate target DOM extraction. Table extraction parsing safely executes with BeautifulSoup.
...
*   `tests/test_table_extractor.py`: Added tests for table extraction logic.
...
5.  **Revert `core/extractors/table_extractor.py`**: Remove `flavor='bs4'` from the `pd.read_html` call.
6.  **Revert Table Tests**: Delete `tests/test_table_extractor.py`.
