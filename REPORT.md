...
*   **P0 — Critical Security**: In `core/extractors/table_extractor.py`, `pandas.read_html` used the default `lxml` parser, which is vulnerable to XML External Entity (XXE) attacks when parsing untrusted HTML data.
...
*   **`core/extractors/table_extractor.py`**:
    ...
    *   **Modification**: Added `flavor='bs4'` to `pd.read_html()` to enforce parsing using the safe BeautifulSoup parser instead of `lxml`.
    *   **Impact & Risks**: High security benefit. Minor compatibility risk if some specific tables were better parsed by `lxml`.
...
*   **Detected vulnerabilities**:
    ...
    *   XML External Entity (XXE) vulnerability due to default `lxml` parser in `pandas.read_html`.
*   **Applied fixes**:
    *   Enforced standard certificate validation in `core/scraper.py`.
    *   Enforced `bs4` parsing by setting `flavor='bs4'` in `core/extractors/table_extractor.py` to mitigate XXE risks.
...
*   **New tests**:
    ...
    *   Added `tests/test_table_extractor.py` to cover table extraction using the `extract_tables` function and verify HTML table parsing with the updated `flavor`.
*   **Updated tests**: N/A
*   **Coverage impact**: Introduced the foundational testing framework. The core cleaning logic is now verified against XSS payload stripping and accurate target DOM extraction, along with HTML table extractions ensuring backward compatibility and functional correctness.
...
*   `core/extractors/table_extractor.py`: Updated `pd.read_html` to use `flavor='bs4'` to prevent XXE vulnerabilities.
*   `tests/test_cleaner.py`: Added new unit tests for HTML cleaning functions.
*   `tests/test_table_extractor.py`: Added tests to ensure tables are correctly extracted.
...
4.  **Revert `core/extractors/table_extractor.py`**: Remove `flavor='bs4'` from the `pd.read_html` call.
