...
*   **P1 — Data Integrity / Security**: In `core/extractors/table_extractor.py`, `pd.read_html` did not specify a parser flavor. Using the default `lxml` parser can expose the application to XML External Entity (XXE) vulnerabilities.
...
*   **`core/extractors/table_extractor.py`**:
    *   **Reason**: Prevent XML External Entity (XXE) vulnerabilities associated with the default `lxml` parser when using `pd.read_html`.
    *   **Modification**: Added `flavor='bs4'` to the `pd.read_html()` call.
    *   **Impact & Risks**: High security benefit. No compatibility risk since `beautifulsoup4` is already a dependency and HTML processing in the project heavily relies on it.
...
*   **Detected vulnerabilities**:
    ...
    *   XML External Entity (XXE) vulnerability in `pd.read_html` via default `lxml` parser.
*   **Applied fixes**:
    *   Enforced standard certificate validation in `core/scraper.py`.
    *   Specified `flavor='bs4'` in `pd.read_html` in `core/extractors/table_extractor.py`.
...
2.  **Revert `core/extractors/table_extractor.py`**: Remove `flavor='bs4'` from the `pd.read_html` arguments.
