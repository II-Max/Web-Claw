...
*   **P0 — Critical Security**: In `core/extractors/table_extractor.py`, `pd.read_html` used the default `lxml` parser, which can be susceptible to XML External Entity (XXE) attacks when parsing untrusted HTML data.
...
*   **`core/extractors/table_extractor.py`**:
    *   **Reason**: Prevent XML External Entity (XXE) vulnerabilities in `pandas.read_html` by enforcing the BeautifulSoup4 parser (`flavor='bs4'`), which is safer for untrusted HTML.
    *   **Modification**: Added `flavor='bs4'` to the `pd.read_html()` call.
    *   **Impact & Risks**: High security benefit. Minimal risk, as `html5lib` is already a requirement.
...
*   **Detected vulnerabilities**:
    ...
    *   XML External Entity (XXE) vulnerability in `pandas.read_html` using the default parser.
*   **Applied fixes**:
    *   Enforced standard certificate validation in `core/scraper.py`.
    *   Enforced safe parsing with `flavor='bs4'` in `core/extractors/table_extractor.py`.
...
*   **New tests**:
    ...
    *   Added `tests/test_table_extractor.py` to cover `extract_tables`.
*   **Updated tests**: N/A
*   **Coverage impact**: Introduced the foundational testing framework. The core cleaning logic is now verified against XSS payload stripping and accurate target DOM extraction. Table extraction is now tested for correct data parsing and output structure.

## Changelog
*   `core/scraper.py`: Removed `verify=False` to fix TLS validation vulnerability.
*   `core/extractors/table_extractor.py`: Added `flavor='bs4'` to prevent XXE attacks in `pd.read_html`.
